...
export OCSVERSION=${35}
export RETRYFLAG=${36}

echo "OPENSHIFT VERSION "  $OCSVERSION
echo "RETRY FLAG " $RETRYFLAG

if [ -z $RETRYFLAG ]; then
    RETRYFLAG="true";
fi

if [ -z $OCSVERSION ]; then
    OCSVERSION="4.7.15";
fi
...
runuser -l $SUDOUSER -c "wget https://mirror.openshift.com/pub/openshift-v4/x86_64/clients/ocp/${OCSVERSION}/openshift-install-linux-${OCSVERSION}.tar.gz"
runuser -l $SUDOUSER -c "wget https://mirror.openshift.com/pub/openshift-v4/x86_64/clients/ocp/${OCSVERSION}/openshift-client-linux-${OCSVERSION}.tar.gz"
runuser -l $SUDOUSER -c "tar -xvf openshift-install-linux-${OCSVERSION}.tar.gz -C $INSTALLERHOME"
runuser -l $SUDOUSER -c "sudo tar -xvf openshift-client-linux-${OCSVERSION}.tar.gz -C /usr/bin"
...
if [[ $STORAGEOPTION == "ocs" ]]; then
  runuser -l $SUDOUSER -c "wget $ARTIFACTSLOCATION/scripts/installOCS.sh$ARTIFACTSTOKEN -O $INSTALLERHOME/openshiftfourx/installOCS.sh"
  runuser -l $SUDOUSER -c "chmod +x $INSTALLERHOME/openshiftfourx/installOCS.sh"
  runuser -l $SUDOUSER -c "sh $INSTALLERHOME/openshiftfourx/installOCS.sh ${CLUSTERNAME} ${DOMAINNAME} ${OPENSHIFTUSER} ${OPENSHIFTPASSWORD}"	
fi
